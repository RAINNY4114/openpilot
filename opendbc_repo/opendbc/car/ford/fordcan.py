...
def create_lat_ctl_msg(packer, CAN: CanBus, lat_active: bool, ramp_type: int, precision_type: int, path_offset: float,
...
def create_lat_ctl2_msg(packer, CAN: CanBus, mode: int, ramp_type: int, precision_type: int, path_offset: float,
...
def create_acc_msg(packer, CAN: CanBus, long_active: bool, gas: float, accel: float, accel_pred: float, stopping: bool,
  ...
  values = {
    ...
    "AccBrkPrchg_B_Rq": 1 if precharge_actuate else 0,        # Pre-charge brake request: 0=No, 1=Yes
    "AccBrkDecel_B_Rq": 1 if brake_actuate else 0,            # Deceleration request: 0=Inactive, 1=Active

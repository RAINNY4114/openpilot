...
class CarState(CarStateBase):
    def __init__(self, CP):
        ...
        self.lidar_left_detected = False
        self.lidar_right_detected = False
        self.lidar_left_objs = []
        self.lidar_right_objs = []
    ...
    def update(self, can_parsers) -> structs.CarState:
        ...
        # ===== Blindspot + LiDAR fusion =====
        if self.CP.enableBsm:
            cp_bsm = cp_cam if self.CP.flags & FordFlags.CANFD else cp
            # 原始 BSM
            bsm_left = cp_bsm.vl["Side_Detect_L_Stat"]["SodDetctLeft_D_Stat"] != 0
            bsm_right = cp_bsm.vl["Side_Detect_R_Stat"]["SodDetctRight_D_Stat"] != 0

            # 激光雷达融合
            lidar_left = False
            lidar_right = False

            if hasattr(self, "lidar_last_update") and time.time() - self.lidar_last_update < 0.3:
                if hasattr(self, "lidar_left_detected"):
                    lidar_left = self.lidar_left_detected
                if hasattr(self, "lidar_right_detected"):
                    lidar_right = self.lidar_right_detected
                if hasattr(self, "lidar_left_objs"):
                    for o in self.lidar_left_objs:
                        if o.get("d", 999) < 10:
                            lidar_left = True
                            break
                if hasattr(self, "lidar_right_objs"):
                    for o in self.lidar_right_objs:
                        if o.get("d", 999) < 10:
                            lidar_right = True
                            break

            # 融合输出
            ret.leftBlindspot = bsm_left or lidar_left
            ret.rightBlindspot = bsm_right or lidar_right

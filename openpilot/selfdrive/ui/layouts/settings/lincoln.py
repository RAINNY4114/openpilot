...
class LincolnLayout(Widget):
  def __init__(self):
    ...
    self._scroller = Scroller([
      ...
      simple_item(title=lambda: tr("### Ford / Lincoln Lateral Tuning ###")),
      double_spin_button_item(
        title=lambda: tr("In-Lane Offset"),
        description=lambda: tr("Adjust the lane-centering offset. Negative values bias left, positive values bias right."),
        initial_value=self._get_param_float("custom_path_offset", 0.0),
        callback=lambda val: self._params.put("custom_path_offset", float(round(val, 2))),
        min_val=-0.50,
        max_val=0.50,
        step=0.05,
        decimals=2,
      ...
      double_spin_button_item(
        ...
        description=lambda: tr("Blend ratio for predicted curvature on straights and gentle curves."),
        initial_value=self._get_param_float("pc_blend_ratio_low_C_UI", 0.40),
        callback=lambda val: self._params.put("pc_blend_ratio_low_C_UI", float(round(val, 2))),
        min_val=0.00,
        max_val=1.00,
        step=0.05,
        decimals=2,
      ...
      double_spin_button_item(
        ...
        description=lambda: tr("Blend ratio for predicted curvature on medium and larger curves."),
        initial_value=self._get_param_float("pc_blend_ratio_high_C_UI", 0.40),
        callback=lambda val: self._params.put("pc_blend_ratio_high_C_UI", float(round(val, 2))),
        min_val=0.00,
        max_val=1.00,
        step=0.05,
        decimals=2,
      ...
      double_spin_button_item(
        ...
        description=lambda: tr("Strength of low-curvature lane-centering correction."),
        initial_value=self._get_param_float("LC_PID_gain_UI", 3.0),
        callback=lambda val: self._params.put("LC_PID_gain_UI", float(round(val, 1))),
        min_val=0.0,
        max_val=5.0,
        step=0.1,
        decimals=1,
      ...
      double_spin_button_item(
        title=lambda: tr("Lane Change Factor High"),
        description=lambda: tr("High-speed lane-change smoothing factor. Lower values soften the lane change more."),
        initial_value=self._get_param_float("lane_change_factor_high", 0.85),
        callback=lambda val: self._params.put("lane_change_factor_high", float(round(val, 2))),
        min_val=0.50,
        max_val=1.00,
        step=0.05,
        decimals=2,

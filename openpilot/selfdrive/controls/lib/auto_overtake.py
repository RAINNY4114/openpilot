...
# Constants for highway auto-overtake (experimental).
...
class AutoOvertakeHelper:
    ...
    @staticmethod
    def get_stable_state(ok: bool, ok_since: float | None, now: float, stable_sec: float) -> tuple[float | None, bool]:
        """Returns the stable state and time for lane change."""
        if ok:
            if ok_since is None:
                ok_since = now
    ...
    def _update_need_overtake(self, *, now: float, lead_present: bool, lead_d: float, v_lead: float,
        ...
        """Check if overtaking is needed based on lead vehicle conditions."""
        if not lead_present or lead_d <= 0.0 or v_lead is None or v_cruise < min_cruise_speed or v_ego < min_speed:
            self._need_since = None
            return False
    ...
    def update(self, *, enabled: bool, lc_state: LaneChangeState, v_ego: float, v_cruise: float,
        ...
        """Main update function to determine lane change requests based on various conditions."""
        now = time.monotonic()
        ...
        if not enabled or not bsm_available or v_ego < min_speed or v_cruise < min_cruise_speed:
            self.reset()
        ...
        self._left_ok_since, left_stable = self.get_stable_state(left_ok, self._left_ok_since, now, CLEAR_LANE_STABLE_SEC)
        self._right_ok_since, right_stable = self.get_stable_state(right_ok, self._right_ok_since, now, CLEAR_LANE_STABLE_SEC)
        ...
        # Idle state: check if overtaking is needed and prepare for lane change
        if self._mode == "idle":
            self._out_dir = LaneChangeDirection.none
            self._return_dir = LaneChangeDirection.none
            self._prepare_since = None
        ...
        # Preparing state: prepare for lane change after a short delay
        ...
        # Changing out: initiate the lane change
        ...
        # Holding: wait for return to the original lane if not staying in the passing lane
        elif self._mode == "holding":
            if not stay_in_pass_lane:
                self._mode = "waiting_return"
                self._clear_since = None

        # Waiting return: wait for conditions to return to the original lane
        ...
        # Changing back: return to the original lane
